...
//			System.out.print(Math.min(prevLarger.get(i), nextLarger.get(i)) - arr.get(i)+ ", ");
	...
	private static ArrayList<Integer> LargestToLeft(ArrayList<Integer> arr) {
		ArrayList<Integer> ans = new ArrayList<>(Collections.nCopies(arr.size(), 0));
		int max = Integer.MIN_VALUE;
		for(int i = 0; i < arr.size(); i++) {
			if(arr.get(i) > max) {
